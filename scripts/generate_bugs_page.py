...
def main():
    ...
    content.append(
        f"## {STATUS_ICON['Confirmed']} Confirmed ({conf + fix}) + {STATUS_ICON['Fixed']} Fixed ({fix}) + {STATUS_ICON['Submitted']} Pending ({sub}) + {STATUS_ICON['Rejected']} Rejected ({total - (sub + conf + fix)}) = {total}"
    )
